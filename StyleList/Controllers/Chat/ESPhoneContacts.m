...
- (void)viewDidLoad {
    ...
    [store requestAccessForEntityType:CNEntityTypeContacts completionHandler:^(BOOL granted, NSError * _Nullable error) {
        if (granted == YES)
        {
            ...
            NSError *error;
            NSArray *cnContacts = [store unifiedContactsMatchingPredicate:nil keysToFetch:keys error:&error];
            if (error)
            {
            ...
            else
            {
                ...
                for (CNContact *contact in cnContacts) {
                    ...
                    for (CNLabeledValue *label in contact.emailAddresses) {
                            email = label.value;
                            if ([email length] > 0) {
                                [emailsArray addObject:[NSString stringWithFormat:@"%@", email]];
                            }
                    }
                    [users1 addObject:@{@"name":fullName, @"emails":emailsArray, @"phones":contactNumbersArray}];
                }
                ...
                [query whereKey:kESUserObjectIdKey doesNotMatchKey:kESBlockedUserID2 inQuery:query1];
