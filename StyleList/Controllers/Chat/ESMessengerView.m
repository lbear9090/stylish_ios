...
- (void)viewDidLoad {
    ...
    [firebase1 observeSingleEventOfType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot *snapshot) {
        [self finishReceivingMessage];
...
- (void)sendMessage:(NSString *)text withPicture:(UIImage *)picture withVideo:(NSURL *)video andWithAudio:(NSString *)audio {
    ...
    //TODO: if the user deleted the chat previously, we need to recreate it
    //[ESUtility recreateConversation:groupId];
...
- (void)eventuallySaveMessage:(NSMutableDictionary *)messengerItem {
    ...
    [reference setValue:messengerItem withCompletionBlock:^(NSError * _Nullable error, FIRDatabaseReference * _Nonnull ref) {
...
- (void)updateFirebaseMessage:(NSDictionary *)messengerItem {
    ...
    if ([defaults boolForKey:@"readreceipt"]) {
        ...
        {
            [[firebase1 childByAppendingPath:messengerItem[@"key"]] updateChildValues:@{@"status":@"Read"} withCompletionBlock:^(NSError * _Nullable error, FIRDatabaseReference * _Nonnull ref) {
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView didTapMessageBubbleAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([message.senderId isEqualToString:self.senderId] == YES) {
  	...
  	if (message.isMediaMessage) {
        ...
        NSMutableArray *ownedTempArr =[[NSMutableArray alloc]init];
        NSMutableArray *tempItemObjIdCount = [[NSMutableArray alloc]init];
		if ([message.media isKindOfClass:[JSQPhotoMediaItem class]]) {
            ...
            JTSImageViewController *imageViewer = [[JTSImageViewController alloc]
                                                   ...
                                                   postId:@"" postUser:tempUser
                                                   registeredItemObjectBookmarkState:tempArr
                                                   registeredOwnedItemObjectState:ownedTempArr
                                                   selectedItemObjectIdArray:tempItemObjIdCount];
