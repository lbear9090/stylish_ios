...
@interface ESMediaViewController () {
    float previousAmount2;
    
}
@end

@implementation ESMediaViewController
...
- (PFQuery *)queryForTable {
    ...
    PFQuery *photosQuery = [PFQuery queryWithClassName:self.parseClassName];
    [photosQuery whereKeyDoesNotExist:@"type"];
    [photosQuery whereKeyExists:kESPhotoPictureKey];
    
    
    PFQuery *videosQuery = [PFQuery queryWithClassName:self.parseClassName];
    [videosQuery whereKeyExists:@"type"];
    [videosQuery whereKeyExists:kESVideoFileKey];
    
    PFQuery *query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects:photosQuery, videosQuery, nil]];
...
- (void)didBtnHashTagClicked:(UIButton *)sender{
    NSLog(@"btnhashtag indexM---%ld" , (long)sender.tag);
}

- (void)didBtnBookmarkClicked:(UIButton *)sender{
    NSLog(@"btnbookmark indexM---%ld", (long)sender.tag);
}
