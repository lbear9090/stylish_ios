...
@synthesize _tableView, switchview, switchviewPush, switchviewRead, switchviewPrivate;
...
- (void)viewDidLoad {
    ...
    tableFooter.font = [UIFont fontWithName:GTWALSHEIM_REGULAR_FONT size:12];
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 3) {
            ...
            cell.textLabel.text = NSLocalizedString(@"Private account", nil);
            cell.selectionStyle = UITableViewCellEditingStyleNone;
            switchviewPrivate = [[UISwitch alloc] initWithFrame:CGRectZero];
            cell.accessoryView = switchviewPrivate;
            [switchviewPrivate addTarget:self action:@selector(switchToggledPrivate) forControlEvents: UIControlEventValueChanged];
            if([[[PFUser currentUser] objectForKey:@"privateAccount"] isEqualToString:@"YES"]){
                [switchviewPrivate setOn:YES animated:YES];
            }
            else {
                [switchviewPrivate setOn:NO animated:YES];
            }
        }
...
- (void) switchToggledPrivate {
    
    if (![switchviewPrivate isOn]) {
        // [switchview setOn:NO animated:YES];
        [[NSUserDefaults standardUserDefaults]setBool:NO forKey:@"privateAccount"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        PFUser *user = [PFUser currentUser];
        [user setObject:@"NO" forKey:@"privateAccount"];
        [user saveInBackground];
        [[UIApplication sharedApplication] unregisterForRemoteNotifications];
        
    } else {
        // [switchview setOn:YES animated:YES];
        [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"privateAccount"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        PFUser *user = [PFUser currentUser];
        [user setObject:@"YES" forKey:@"privateAccount"];
        [user saveInBackground];
        UIUserNotificationSettings *settings = [UIUserNotificationSettings settingsForTypes:(UIRemoteNotificationTypeBadge
                                                                                             |UIRemoteNotificationTypeSound
                                                                                             |UIRemoteNotificationTypeAlert) categories:nil];
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    switch (section) {
        case 0:
            ...
            break;
        case 1:
            return NSLocalizedString(@"Alerts & Notifications", nil);
            break;
        case 2:
            return @"";
            break;
...
- (void)tableView:(UITableView *)aTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (indexPath.section == 0) {
      if (indexPath.row == 1) {
          ESBlockedView *blockedView = [[ESBlockedView alloc] initWithNibName:nil bundle:nil];
    ...
    else if (indexPath.section == 1)
    {
        ...
        TOWebViewController *webViewController = [[TOWebViewController alloc] initWithURL:[NSURL URLWithString:WEBSITE_URL]];
        webViewController.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:webViewController animated:YES];
     }
