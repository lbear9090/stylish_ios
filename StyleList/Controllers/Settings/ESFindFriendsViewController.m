...
@synthesize headerView,searchQuery,searchQuery2,orQuery;
...
-(void)filterResults:(NSString *)searchTerm {
    ...
    [query1 whereKey:kESBlockedUser1 equalTo:[PFUser currentUser]];
    
    searchQuery = [PFUser query];
    [searchQuery whereKey:kESUserObjectIdKey doesNotMatchKey:kESBlockedUserID2 inQuery:query1];
    ...
    searchQuery2 = [PFUser query];
    [searchQuery2 whereKey:kESUserObjectIdKey doesNotMatchKey:kESBlockedUserID2 inQuery:query1];
    [searchQuery2 whereKeyExists:kESUserMentionNameKey];  //this is based on whatever query you are trying to accomplish
    [searchQuery2 whereKeyExists:@"username"]; //this is based on whatever query you are trying to accomplish
    [searchQuery2 whereKey:kESUserMentionNameKey containsString:[searchTerm lowercaseString]];
    [searchQuery2 whereKey:kESUserObjectIdKey notEqualTo:[[PFUser currentUser] objectId]];

    orQuery = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects:searchQuery, searchQuery2, nil]];
    [self.orQuery findObjectsInBackgroundWithBlock:^(NSArray
...
- (PFQuery *)queryForTable {
    ...
    [friendsQuery whereKey:kESUserObjectIdKey doesNotMatchKey:kESBlockedUserID2 inQuery:query1];
...
- (void)presentMailComposeViewController:(NSString *)recipient {
    ...
    [composeEmailViewController setMessageBody:@"<h2>Share your pictures, share your story.</h2>" isHTML:YES];
...
- (void)presentMessageComposeViewController:(NSString *)recipient {
    ...
    [composeTextViewController setBody:NSLocalizedString(@"Check out :)", nil)];
