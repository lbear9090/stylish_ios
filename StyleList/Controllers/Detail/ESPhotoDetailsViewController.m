...
@synthesize photo, isText, headerView;
...
- (id)initWithPhoto:(PFObject *)aPhoto  andTextPost:(NSString *)isIt {
...
- (void)viewDidLoad {
    ...
    // Set table header
    if ([[self.photo objectForKey:@"type"]isEqualToString:@"text"] || [[self.photo objectForKey:@"type"] isEqualToString:@"retweet"] || [self.isText isEqualToString:@"YES"] || [self.isText isEqualToString:@"YES-RETWEET"]) {
        ...
        if ( [[self.photo objectForKey:@"type"] isEqualToString:@"retweet"] || [self.isText isEqualToString:@"YES-RETWEET"]) {
            self.headerView = [[ESPhotoDetailsHeaderView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 46+labelHeight+30+63) photo:self.photo];
        }
        else {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"More_Options_64"] style:UIBarButtonItemStyleDone target:self action:@selector(shareButtonAction:)];
...
- (PFQuery *)queryForTable {
    ...
    if ([[self.photo objectForKey:@"type"] isEqualToString:@"text"] || [[self.photo objectForKey:@"type"] isEqualToString:@"retweet"]) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (trimmedComment.length != 0 && [self.photo objectForKey:kESPhotoUserKey]) {
        ...
        }else if ([[photo objectForKey:@"type"] isEqualToString:@"text"] || [[self.photo objectForKey:@"type"] isEqualToString:@"retweet"]) {
...
- (void)photoDetailsHeaderView:(ESPhotoDetailsHeaderView *)_headerView didTapPhotoButton:(UIButton *)button {
    ...
    imageInfo.image = self.headerView.photoImageView.image;
    ...
    JTSImageViewController *imageViewer = [[JTSImageViewController alloc]
                                           ...
                                           postId:self.photo.objectId postUser:[self.photo objectForKey:kESPhotoUserKey]
                                           registeredItemObjectBookmarkState:nil
                                           registeredOwnedItemObjectState:nil
                                           selectedItemObjectIdArray:nil];
...
- (void)shareButtonAction:(id)sender {
    ...
    if (([[self.photo objectForKey:@"type"]isEqualToString:@"text"] || [[self.photo objectForKey:@"type"] isEqualToString:@"retweet"] || [self.isText isEqualToString:@"YES"])) {
        [actionSheet addButtonWithTitle:NSLocalizedString(@"Repost", nil)];
    }
...
- (void)activityButtonAction:(id)sender {
    if (NSClassFromString(@"UIActivityViewController")) {
        ...
        if ([[self.photo objectForKey:kESPhotoPictureKey] isDataAvailable]) {
            [self showShareSheet];
        } else if ([[self.photo objectForKey:@"type"] isEqualToString:@"text"] || [[self.photo objectForKey:@"type"] isEqualToString:@"retweet"]) {
            [self showShareSheet];
        }
        else {
...
- (void) retweetPost {
    ...
    PFUser *op = [self.photo objectForKey:kESPhotoUserKey];
    PFObject *post = [PFObject objectWithClassName:@"Photo"];
    
    NSString *text = [NSString stringWithFormat:@"%@",[self.photo objectForKey:@"text"]];
    [post setObject:@"retweet" forKey:@"type"];
    PFACL *defaultACL = [PFACL ACL];
    [defaultACL setPublicReadAccess:YES];
    [defaultACL setPublicWriteAccess:YES];
    [post setACL:defaultACL];
    [post setObject:text forKey:@"text"];
    [post setObject:op forKey:@"retweetedUser"];
    ...
    [post saveInBackgroundWithBlock:^(BOOL result, NSError *error){
        if (!error) {
            ...
            [ProgressHUD showSuccess:@"Retweeted!"];
        }
        else {
            [ProgressHUD showError:@"Could not be posted!"];
            NSLog(@"%@",error);
        }
    }];
...
- (void)loadLikers {
    ...
    if ([[photo objectForKey:@"type"] isEqualToString:@"text"] || [[self.photo objectForKey:@"type"] isEqualToString:@"retweet"])
    {
...
- (void)useNotificationWithMentionString:(NSNotification *)notification {
    ...
    [userQuery whereKey:@"usernameFix" equalTo:[stringValueToUse lowercaseString]];
