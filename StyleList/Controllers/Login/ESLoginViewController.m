...
@synthesize fieldEmail, fieldPassword,cellFacebook, presetMail;
...
- (void)viewDidLoad
{
    ...
    //this is test code
//    fieldEmail.text = @"jangsan603@hotmail.com";
//    fieldPassword.text = @"jangsan";
    ...
    [PFUser logInWithUsernameInBackground:email password:password block:^(PFUser *user, NSError *error)
     {
         if (user != nil)
         {
             ...
             if([[user objectForKey:@"emailVerified"] boolValue]) {
                 ...
                 [ProgressHUD showError:@"Email not verified.\n\nYou need to verify it by clicking the link we sent you in the email."];
                 [PFUser logOut];
             }
#endif
         }
...
- (void)actionFacebookLogin {
    ...
    [PFFacebookUtils logInInBackgroundWithReadPermissions:@[@"user_about_me", @"public_profile", @"email", @"user_friends"] block:^(PFUser * _Nullable user, NSError * _Nullable error) {
         if (user != nil)
         {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if(presetMail.length != 0) {
            fieldEmail.text = self.presetMail;
        }
        ...
        facebookLogin.titleLabel.font = [UIFont fontWithName:GTWALSHEIM_REGULAR_FONT size:16];
