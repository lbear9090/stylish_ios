...
#import "ESMakeFeaturedActivity.h"
...
@interface ESPhotoTimelineViewController ()<MFMailComposeViewControllerDelegate> {
    NSMutableArray *trendingAry;
}
@end
...
-(void)loadView {
    ...
    trendingAry = [[NSMutableArray alloc] init];
    [self findTrendingObjects];
}

- (void) findTrendingObjects {
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray * objects, NSError * _Nullable error) {
        [trendingAry addObjectsFromArray:objects];
    }];
}
...
- (void)viewDidLoad {
    ...
    UIColor *backgroundColor = [UIColor colorWithWhite:1 alpha:1];
    ...
    [loadMoreView setBackgroundColor:[UIColor redColor]];
...
- (void)addObjects:(NSString*)selectedObject{
    
    ESCache *shared = [ESCache sharedCache];
    shared.tempObjects = [self.objects copy];
    self.tempObject = [[NSMutableArray alloc]init];
    PFUser *user = [PFUser currentUser];
    //============== style tag for recent tab =======================//
    NSArray *styleArray = [[NSArray alloc] init];
    styleArray = [user objectForKey:@"Style"];
    //============== hash tag, own tag , like tag for trending tab ==//
    
    //============== following count for featured tab ===============//
    NSMutableDictionary *dict = [[NSMutableDictionary alloc]init];
    
    if ([selectedObject isEqualToString:RECENT]) {
        ...
        if (styleArray.count>0) {
            ...
            for (int i = 0; i<self.objects.count; i++) {
                ...
                NSString *str = [[dict objectForKey:kESPhotoStyleTags] firstObject]; //.//??
                
                for (int j = 0; j<styleArray.count; j++){
                    NSString *currentUserStyleTag = [styleArray objectAtIndex:j];
                    if ([str isEqualToString:currentUserStyleTag]) {
            ...
            [self.objects removeAllObjects];
            [self.objects addObjectsFromArray:shared.tempObjects];
        }
    }
    if ([selectedObject isEqualToString:TRENDING]) {
        
        /*if (shared.likedUserPostObjects.count > 0) {
         for (int i = 0; i < self.objects.count; i++) {
         PFObject *object = [self.objects objectAtIndex:i];
         NSString *objId = object.objectId;
         for (int j = 0; j < shared.likedUserPostObjects.count; j++) {
         PFObject *object = [shared.likedUserPostObjects objectAtIndex:j];
         PFObject *postObject = [object objectForKey:kESActivityPhotoKey];
         NSString *strPostObjectId = postObject.objectId;
         if ([objId isEqualToString:strPostObjectId]) {
         [self.tempObject addObject:[self.objects objectAtIndex:i]];
         }
         }
         }
        ...
        [self.objects removeAllObjects];
        [self.objects addObjectsFromArray:trendingAry];
    }
    if ([selectedObject isEqualToString:FEATURED]) {
        [self.objects removeAllObjects];
        if (shared.followedUserPostObjects.count >0) {
            [self.objects addObjectsFromArray:shared.followedUserPostObjects];
        }
        
    }
    //============== module for recent tab ===============//
    /* NSMutableDictionary *dict = [[NSMutableDictionary alloc]init];
     ...
     NSInteger length = [timestamp length];
     if (length>3) {
     NSInteger tempLength = length - 4;
     NSString *sortTime = [timestamp substringWithRange:NSMakeRange(tempLength, 4)];
     
     if (![sortTime isEqualToString:@"days"] || [timestamp isEqualToString:@"1day"]) {
...
- (PFQuery *)queryForTable {
    ...
    //recent
    PFQuery *query = [PFQuery queryWithClassName:self.parseClassName];
    //    [query whereKey:@"HashTag" equalTo:@"jacket"]; //test
    [query includeKey:kESPhotoUserKey];
    [query orderByDescending:@"createdAt"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    ...
    /* else if (!(indexPath.section % 3) && indexPath.section != 0) {
     ...
     return cell;
        ...
        // cell.imageView.hidden= YES;
        if (object) {
            CGSize labelSize = [[object objectForKey:@"text"] sizeWithFont:[UIFont fontWithName:GTWALSHEIM_REGULAR_FONT size:16]
                                                         constrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width - 20, 100)
                                                             lineBreakMode:NSLineBreakByWordWrapping];
            CGFloat labelHeight = labelSize.height;
    ...
    if ([photo objectForKey:kESVideoFileKey]) {
      ...
      /* [[photo objectForKey:kESPhotoPictureKey] getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
            if (!error) {
                ...
                UIActivityViewController *activityViewController = [[UIActivityViewController alloc] initWithActivityItems:activityItems applicationActivities:nil];
                activityViewController.excludedActivityTypes = @[UIActivityTypeMail, UIActivityTypePostToTwitter, UIActivityTypePostToVimeo, UIActivityTypePostToFacebook];
                if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")) {
                    activityViewController.popoverPresentationController.sourceView = self.navigationController.navigationBar;
                }
                dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSString *postingUserId = postingUser.objectId;
        
        ESMessageSellerActivity     *msgACV         = [[ESMessageSellerActivity alloc] init];
        ESReportItemActivity        *reportACV      = [[ESReportItemActivity alloc] init];
        //ESMakeFeaturedActivity      *mFeaturedACV   = [[ESMakeFeaturedActivity alloc] init];
        //ESMarkItemSoldActivity      *mItemSoldACV   = [[ESMarkItemSoldActivity alloc]init];
        ESDeleteItemActivity        *deleteACV      = [[ESDeleteItemActivity alloc]init];
        
        msgACV.delegate         = self;
        reportACV.delegate      = self;
        //mFeaturedACV.delegate   = self;
        //mItemSoldACV.delegate   = self;
        deleteACV.delegate      = self;
        
        NSArray *appCommonPostACV     = @[msgACV, reportACV];
        //NSArray *appOwnPostACV        = @[mFeaturedACV, mItemSoldACV, deleteACV];
        NSArray *appOwnPostACV        = @[deleteACV];
//        NSArray *activitiesItemsForCommonPostACV = @[@"msgACV", @"reportACV"];
//        NSArray *activitiesItemsForOwnPostACV    = @[@"mFeaturedACV", @"mItemSoldACV",@"deleteACV"];
        ...
        [[photo objectForKey:kESPhotoPictureKey] getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
            if (!error) {
                ...
                [activityItems addObject:[UIImage imageWithData:data]];
                [activityItems addObject:photo];
                
                if ([currentUserId isEqualToString:postingUserId]) {
                     activityVC = [[UIActivityViewController alloc] initWithActivityItems:activityItems applicationActivities:appOwnPostACV];
                  
                }else{
                     activityVC = [[UIActivityViewController alloc] initWithActivityItems:activityItems applicationActivities:appCommonPostACV];
                }
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray * _Nullable objects, NSError * _Nullable error){
        ...
        if (!error) {
            ...
            if (photo) {
                ...
                //                [self showFullPhotoItemView:photoFile postId:strPostObjectId postUser:postingUser];
            }
        }else{
            NSLog(@"error --- ");
            if (photo) {
...
#pragma mark - get itemobject of type(owned) from server.
...
- (void)didBtnHashTagClicked:(UIButton *)sender{
    NSLog(@"btnhashtag index PT---%d" , sender.tag);
    //    PFObject *object = [self.objects objectAtIndex:sender.tag];
    //    ESBuyViewController *vc = [[ESBuyViewController alloc]initWithPhoto:object];
    //    UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:vc];
    //    [self presentViewController:navController animated:YES completion:nil];
}

- (void)didBtnBookmarkClicked:(UIButton *)sender{
    
    NSLog(@"btnbookmark index PT---%d", sender.tag);
}
