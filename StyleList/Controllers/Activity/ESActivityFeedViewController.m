...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < self.objects.count) {
        ...
        if ([activity objectForKey:kESActivityPhotoKey]) {
            ...
            else if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeMentionPost]) {
                ESPhotoDetailsViewController *photoViewController = [[ESPhotoDetailsViewController alloc] initWithPhoto:photo  andTextPost:@"YES"];
                [self.navigationController pushViewController:photoViewController animated:YES];
            }
            else {
                ESPhotoDetailsViewController *photoViewController = [[ESPhotoDetailsViewController alloc] initWithPhoto:photo andTextPost:@"NO"];
                [self.navigationController pushViewController:photoViewController animated:YES];
            }
...
- (void)cell:(ESActivityCell *)cellView didTapActivityButton:(PFObject *)activity {
    ...
    PFObject *photo = [activity objectForKey:kESActivityPhotoKey];
    if ([[photo objectForKey:kESVideoOrPhotoTypeKey] isEqualToString:kESVideoTypeKey]) {
        ESVideoDetailViewController *photoViewController = [[ESVideoDetailViewController alloc] initWithPhoto:photo];
        [self.navigationController pushViewController:photoViewController animated:YES];
    }
    else if ([[activity objectForKey:kESActivityTypeKey] isEqualToString:kESActivityTypeMentionPost]) {
...
+ (NSString *)stringForActivityType:(NSString *)activityType {
    ...
    } else if ([activityType isEqualToString:kESActivityTypeMentionPost]) {
