...
@implementation CLDrawTool
{
    ...
    NSMutableArray *arrFromPoint;
    NSMutableArray *arrToPoint;
...
- (void)setup
{
    arrFromPoint = [[NSMutableArray alloc]init];
    arrToPoint = [[NSMutableArray alloc]init];
...
- (void)executeWithCompletionBlock:(void (^)(UIImage *, UIImage *,UIBezierPath *, NSError *, NSDictionary *))completionBlock
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage *image = [self buildImage];
        dispatch_async(dispatch_get_main_queue(), ^{
            completionBlock(image,nil,_aPath, nil, nil);
        });
    });
}
...
- (void)drawingViewDidPan:(UIPanGestureRecognizer*)sender
{
    ...
    if(sender.state == UIGestureRecognizerStateBegan){
        ...
        _firstDragginPosition = currentDraggingPosition;
        _aPath = [UIBezierPath bezierPath];
        // Set the starting point of the shape.
        [_aPath moveToPoint:_firstDragginPosition];
    }
    
    if (sender.state != UIGestureRecognizerStateEnded){
        [self drawLine:_prevDraggingPosition to:currentDraggingPosition];
    } else {
        [self drawLine:currentDraggingPosition to:_firstDragginPosition];
...
-(void)drawLine:(CGPoint)from to:(CGPoint)to
{
    ...
    [_aPath addLineToPoint:CGPointMake(to.x, to.y)];
