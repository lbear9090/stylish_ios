...
- (IBAction)pushedDoneBtn:(id)sender
{
    ...
    [self.currentTool executeWithCompletionBlock:^(UIImage *image, UIImage *bImage, UIBezierPath *drawPath, NSError *error, NSDictionary *userInfo) {
        if(error){
...
//            self.bImage = bImage;
            [_imageView addSubview:[[UIImageView alloc] initWithImage:[self drawBezierArea:drawPath]]];
            _originalImage = [self renderWholeImageView:_imageView];
...
- (UIImage*)drawBezierArea:(UIBezierPath*)path {
    CAShapeLayer *square = [CAShapeLayer layer];
    square.path = path.CGPath;
    square.fillColor = [[UIColor colorWithRed:0.0f/255.0f green:0.0f/255.0f blue:255.0f/255.0f alpha:0.3f] CGColor];
    
    square.frame = self.imageView.bounds;
//    [self.imageView.layer addSublayer:square];
    
    UIGraphicsBeginImageContextWithOptions(square.frame.size, NO, 0);
    
    [square renderInContext:UIGraphicsGetCurrentContext()];
