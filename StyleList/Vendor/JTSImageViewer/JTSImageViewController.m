...
@synthesize btnBinocular, selectBinocular, arrRealPosition, itemView, arrItemView, originalImage;
@synthesize selectedPhotoPostId,selectedPostingUser;
@synthesize arrSelectedItemObjectIdAndName;
...
- (instancetype)initWithImageInfo:(JTSImageInfo *)imageInfo
                  ...
                  backgroundStyle:(JTSImageViewControllerBackgroundOptions)backgroundOptions
                           postId:(NSString *)photoPostId
                         postUser:(PFUser *)postingUser
registeredItemObjectBookmarkState:(NSMutableArray *)regItemObjBookmarkState
   registeredOwnedItemObjectState:(NSMutableArray *)regOwnedItemObjState
        selectedItemObjectIdArray:(NSMutableArray *)selectedItemObjIdArr
{
    ...
    if (self) {
        ...
        //============ init owned, bookmarked item object array =========//
        arrSelectedItemObjectIdAndName = [[NSMutableArray alloc]init];
        arrSelectedItemObjectIdAndName = [selectedItemObjIdArr mutableCopy];
        NSInteger   lblOwnCount        = 0;
        //============ init binocular button frame ======================//
        selectedPhotoPostId = photoPostId;
        selectedPostingUser = postingUser;
        PFUser *currentUser = [PFUser currentUser];
        NSString *strCurrentUserObjectId = currentUser.objectId;
        NSString *strCurrentUserCountry = [currentUser objectForKey:kESUserCountryKey];
        NSString *strPostingUserCountry = [selectedPostingUser objectForKey:kESUserCountryKey];
        
        float tempHeight = (self.view.frame.size.height - self.imageView.frame.size.height)/2;
        float tempWidth = self.view.frame.size.width - (self.view.frame.size.width-self.imageView.frame.size.width)/2-60.0f;
        btnBinocular = [[UIButton alloc]initWithFrame:CGRectMake(tempWidth, tempHeight, 60.0f, 60.0f)];
        [btnBinocular setImage:[UIImage imageNamed:@"binocular.png"] forState:UIControlStateNormal];
        [btnBinocular addTarget:self action:@selector(btnBinoculrClicked) forControlEvents:UIControlEventTouchUpInside];
        self.selectBinocular = NO;
        
        [self.view addSubview:btnBinocular];
        
        NSMutableDictionary *arrDict = [[NSMutableDictionary alloc]init];
        arrItemView         = [[NSMutableArray alloc]init];
        arrRealPosition     = [[NSMutableArray alloc]init];
        
        [self logFrameSize];
        //------- selected item object  -----------//
        ESCache *shared = [ESCache sharedCache];
        [shared.arrOwnLabel removeAllObjects];
        for (int i = 0; i<shared.selectedItemObjects.count; i++) {
            
            NSDictionary *dict = [shared.selectedItemObjects objectAtIndex:i];
            PFObject     *selectItemObject = [shared.selectedItemObjects objectAtIndex:i];
            NSString     *selectItemObjectId = selectItemObject.objectId;
            
            
            float ratioX = [[dict objectForKey:kESPostItemXPosition]floatValue];
            float ratioY = [[dict objectForKey:kESPostItemYPosition]floatValue];
            float ratioRectWidth = [[dict objectForKey:kESPostItemRatioWidth]floatValue];
            float ratioRectHeight = [[dict objectForKey:kESPostItemRatioHeight]floatValue];
            float ratioItemEdgeX = [[dict objectForKey:kESPostItemEdgeRatioXPosition]floatValue];
            float ratioItemEdgeY = [[dict objectForKey:kESPostItemEdgeRatioYPosition]floatValue];
         
            float realX = self.imageView.frame.size.width * ratioX;
            float realY = self.imageView.frame.size.height * ratioY;
            float realRectWidth = self.view.frame.size.width * ratioRectWidth;
            float realRectHeight = self.view.frame.size.height * ratioRectHeight;
            float centerX = self.view.frame.size.width * ratioItemEdgeX;
            float centerY = self.view.frame.size.height * ratioItemEdgeY;
         
            /*float centerX = realX + realRectWidth/2;
            float centerY = realY + realRectHeight/2;*/
            
            [arrDict setObject:[NSString stringWithFormat:@"%f", realX] forKey:kESPostItemXPosition];
            [arrDict setObject:[NSString stringWithFormat:@"%f", realY] forKey:kESPostItemYPosition];
            [arrDict setObject:[NSString stringWithFormat:@"%f", realRectWidth] forKey:kESPostItemRatioWidth];
            [arrDict setObject:[NSString stringWithFormat:@"%f", realRectHeight] forKey:kESPostItemRatioHeight];
            [arrDict setObject:[NSString stringWithFormat:@"%f", centerX] forKey:kESPostItemCenterXPosition];
            [arrDict setObject:[NSString stringWithFormat:@"%f", centerY] forKey:kESPostItemCenterYPosition];
            
            [arrRealPosition addObject:[arrDict mutableCopy]];
            
            //=========== init item view ==========//
            
            itemView = [[ESItemTagView alloc]init:CGRectMake(centerX, centerY, 140.0f, 30.0f)];
            
            itemView.delegate           = self;
            itemView.btnHashTag.tag     = i;
            itemView.lblOwnTag.tag      = i;
            
            NSString *OwnTag            = [NSString stringWithFormat:@"%ld", (long)itemView.lblOwnTag.tag];
            
            NSMutableDictionary *dictOwnTagCount = [[NSMutableDictionary alloc]init];
            [dictOwnTagCount setObject:OwnTag forKey:kESOwnedLabelTag];
            NSString *itemObjId = @"";
            NSString *fromUserObjectId = @"";
            //=========== registered item object owned state ============//
            lblOwnCount = 0; //------ init-------//
            for (int j = 0; j < arrSelectedItemObjectIdAndName.count; j++) {
                
                NSDictionary *dictItemObj = [arrSelectedItemObjectIdAndName objectAtIndex:j];
                itemObjId = [dictItemObj objectForKey:kESActivityItemObjectIdKey];
                fromUserObjectId = [dictItemObj objectForKey:kESActivityFromUserKey];
            
                if ([selectItemObjectId isEqualToString:itemObjId]) {
                    lblOwnCount++;
                    if ([fromUserObjectId isEqualToString:strCurrentUserObjectId]) {
                        [dictOwnTagCount setObject:@"Yes" forKey:kESOwnState];
                    }
                }
            ...
            NSString *ownCount = [NSString stringWithFormat:@"%ld", (long)lblOwnCount];
            [dictOwnTagCount setObject:ownCount forKey:kESOwnedCount];
            //==============================================================//
            //=========== registered item object bookmarked state ==========//
            if (regItemObjBookmarkState.count>0) {
                
                for (int k = 0; k < regItemObjBookmarkState.count; k++) {
                    
                    NSDictionary *dictItemBookmark = [regItemObjBookmarkState objectAtIndex:k];
                    NSString *itemObjId = [dictItemBookmark objectForKey:kESActivityItemObjectIdKey];
                    PFUser   *fromUser = [dictItemBookmark objectForKey:kESActivityFromUserKey];
                    NSString *fromUserObjectId = fromUser.objectId;
                    if ([selectItemObjectId isEqualToString:itemObjId] && [fromUserObjectId isEqualToString:strCurrentUserObjectId]) {
                        [dictOwnTagCount setObject:@"Yes" forKey:kESBookmarkState];
                    }
                }
            }
            ...
            /*NSString     *deliveryMeet      = [selectItemObject objectForKey:kESPostItemDeliveryMeet];
            NSString     *deliveryShipping  = [selectItemObject objectForKey:kESPostItemDeliveryShipping];
            NSString     *domesticCost      = [selectItemObject objectForKey:kESPostItemDomesticCost];
            NSString     *internationalCost = [selectItemObject objectForKey:kESPostItemInternationalCost];
            
            if ([deliveryMeet isEqualToString:@"No"] && [deliveryShipping isEqualToString:@"No"]) {
                
            }else if([deliveryMeet isEqualToString:@"Yes"] && [deliveryShipping isEqualToString:@"No"]){
                if ([strCurrentUserCountry isEqualToString:strPostingUserCountry]) {
                    [arrItemView addObject:itemView];
                    [shared.arrOwnLabel addObject:dictOwnTagCount];
                }
                
            }else if ([deliveryMeet isEqualToString:@"No"] && [deliveryShipping isEqualToString:@"Yes"]){
                if ((domesticCost != nil || ![domesticCost isEqualToString:@""]) && ([internationalCost isEqualToString:@""] || internationalCost == nil)) {
                    if ([strCurrentUserCountry isEqualToString:strPostingUserCountry]) {
                        [arrItemView addObject:itemView];
                        [shared.arrOwnLabel addObject:dictOwnTagCount];
                    }
            ...
            }else if([deliveryMeet isEqualToString:@"Yes"] && [deliveryShipping isEqualToString:@"Yes"]){
                [arrItemView addObject:itemView];
                [shared.arrOwnLabel addObject:dictOwnTagCount];
            }*/
            [arrItemView addObject:itemView];
        ...
        NSLog(@"selected item object count --- %lu", (unsigned long)shared.selectedItemObjects.count);
        NSLog(@"arr dict count --- %lu", (unsigned long)arrDict.count);
        NSLog(@"arrRealPosition  count --- %lu", (unsigned long)arrRealPosition.count);
        
    }
    return self;
}
#pragma get owned count from item object

#pragma set binocular button.
- (void)btnBinoculrClicked{
    if (selectBinocular == NO) {
        [btnBinocular setImage:[UIImage imageNamed:@"selectedBinocular.png"] forState:UIControlStateNormal];
        selectBinocular = YES;
        self.imageView.image = originalImage;
        
        [self showAllItemView];
    }else if (selectBinocular == YES){
        [btnBinocular setImage:[UIImage imageNamed:@"binocular.png"] forState:UIControlStateNormal];
        selectBinocular = NO;
...
#pragma ESItemViewDelegate method.

- (void)hashTagButtonClicked:(ESItemTagView *)view hashTagBtnTag:(UIButton *)buttonTag {
    
    NSInteger tag = buttonTag.tag;
    ESCache *shared = [ESCache sharedCache];
    PFObject *object = [shared.selectedItemObjects objectAtIndex:tag];
    NSString *itemObjectId = object.objectId;
    
    NSMutableArray *arrOwnedItemObject = [[NSMutableArray alloc]init];
    NSMutableArray *arrBookmarkedItemObject = [[NSMutableArray alloc]init];
    
    ESBuyViewController *vc = [[ESBuyViewController alloc]initWithPhoto:object objectId:itemObjectId postPhotoId:selectedPhotoPostId toUser:selectedPostingUser  regOwnedItemObject:arrOwnedItemObject regBookmarkedItemObject:arrBookmarkedItemObject  tag:tag originImg:originalImage];
    UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:vc];
    [self presentViewController:navController animated:YES completion:nil];
    NSLog(@"clicked hashTag item --- %ld", (long)tag);
    
}

#pragma show all item view.
- (void)showAllItemView{
    
    [itemView removeFromSuperview];
    ESCache *shared  = [ESCache sharedCache];
    NSDictionary *dict = [[NSDictionary alloc]init];
    NSMutableDictionary *dictOwnInfo = [[NSMutableDictionary alloc]init];
    
    for (int i = 0; i<arrItemView.count; i++) {
        
        itemView = [arrItemView objectAtIndex:i];
        
        dict = [shared.selectedItemObjects objectAtIndex:i];
        NSString *saleType = [dict objectForKey:kESPostItemSaleType];
        itemView.tagLabel.text = [dict objectForKey:kESPostItemHashTags];
        NSString *strItemPrice = [dict objectForKey:kESPostItemPrice];
        
        if ([strItemPrice isEqualToString:@""] || [strItemPrice isEqualToString:@"(null)"] || strItemPrice == nil) {
            itemView.costtagLabel.text = @"";
        }else{
            if ([saleType isEqualToString:@"For Hire"]) {
                itemView.costtagLabel.text = [NSString stringWithFormat:@"HIRE $%@",[dict objectForKey:kESPostItemPrice]];
            }else if ([saleType isEqualToString:@"For Sale"]){
                itemView.costtagLabel.text = [NSString stringWithFormat:@"BUY $%@",[dict objectForKey:kESPostItemPrice]];
            }else{
                itemView.costtagLabel.text = @"";
            }
        ...
        dictOwnInfo        = [shared.arrOwnLabel objectAtIndex:i];
        // updated by //.//
        NSString *num = [dictOwnInfo objectForKey:kESOwnedCount];
        NSString *ownCount = [NSString stringWithFormat:@"%@ People own this",num];
        if ([num isEqualToString:@"0"]) {
            ownCount = @"";
        } else if([num isEqualToString:@"1"]) {
            ownCount = [NSString stringWithFormat:@"%@ Person owns this",num];
        }
        [itemView.lblOwnTag setText:ownCount];
...
- (void)hideAllItemView{
    ...
    for (int i = 0; i<arrItemView.count; i++) {
        itemView = [arrItemView objectAtIndex:i];
        NSLog(@"item view tag --- %ld",(long)itemView.tag);
        [itemView  removeFromSuperview];
    }
...
- (void)showFromViewController:(UIViewController *)viewController
    ...
    if(arrItemView.count <= 0)
    {
        [btnBinocular setHidden:YES];
    }else{
        [btnBinocular setHidden:NO];
    }
...
- (void)imageSingleTapped:(id)sender {
    ...
    CGPoint touchViewPosition       = [sender locationInView:self.view];
    CGPoint touchImageViewPosition  = [sender locationInView:self.imageView];
    NSLog(@"image view x-----%f, image view y-------%f", touchImageViewPosition.x, touchImageViewPosition.y);
    NSLog(@"view x-----%f, view y-------%f", touchViewPosition.x, touchViewPosition.y);
   
    [self displayViewOfTouchingPosition:touchImageViewPosition.x touchImgViewYPoistion:touchImageViewPosition.y touchViewXPosition:touchViewPosition.x touchViewYPosition:touchViewPosition.y];
//    [self dismiss:YES];
}

#pragma display view of touching position
- (void)displayViewOfTouchingPosition:(CGFloat)touchImgViewX touchImgViewYPoistion:(CGFloat)touchImgViewY touchViewXPosition:(CGFloat)touchViewX touchViewYPosition:(CGFloat)touchViewY{
    
    float viewWidth         = self.view.frame.size.width;
    float viewHeight        = self.view.frame.size.height;
    float imageViewWidth    = self.imageView.frame.size.width;
    float imageViewHeight   = self.imageView.frame.size.height;
    float tempViewWidth     = (viewWidth - imageViewWidth)/2;
    float tempViewHeight    = (viewHeight - imageViewHeight)/2;
    //=========== view width > imageview width ==========//
    if (viewWidth > imageViewWidth) {
        if ((touchViewX>0.0f && touchViewX < viewWidth - imageViewWidth - tempViewWidth) || (touchViewX > viewWidth - tempViewWidth && touchViewX < viewWidth)) {
            [self dismiss:YES];
        }
    }
    //========== view height > imageview height =========//
    if (viewHeight > imageViewHeight) {
        if ((touchViewY >0.0f && touchViewY < viewHeight - imageViewHeight - tempViewHeight) || (touchViewY > viewHeight - tempViewHeight && touchViewY < viewHeight) ) {
            [self dismiss:YES];
        }
    }
    //========== view width, height =  imageview width, height ======//
    if (viewWidth == imageViewWidth && viewHeight == imageViewHeight) {
       
    }
    //========== view width, height < imageview width, height =======//
    if (viewWidth < imageViewWidth && viewHeight < imageViewHeight) {
        if (((touchViewX>0.0f && touchViewX < viewWidth - imageViewWidth - tempViewWidth) || (touchViewX > viewWidth - tempViewWidth && touchViewX < viewWidth)) && ((touchViewY >0.0f && touchViewY < viewHeight - imageViewHeight - tempViewHeight) || (touchViewY > viewHeight - tempViewHeight && touchViewY < viewHeight) )) {
            [self dismiss:YES];
        }
    }
    //================================================================//
    ...
    ESCache *shared = [ESCache sharedCache];
    NSDictionary *dict = [[NSDictionary alloc]init];
    NSDictionary *dict_2 = [[NSDictionary alloc]init];
    NSMutableDictionary *dictOwnInfo = [[NSMutableDictionary alloc]init];
    
    for(int i = 0; i<arrItemView.count; i++){
        
        dict    = [arrRealPosition objectAtIndex:i];
        dict_2  = [shared.selectedItemObjects objectAtIndex:i];
        
        float itemXPosition = [[dict objectForKey:kESPostItemXPosition]floatValue];
        float itemYPosition = [[dict objectForKey:kESPostItemYPosition]floatValue];
        float itemRectWidthPosition = [[dict objectForKey:kESPostItemRatioWidth]floatValue];
        float itemRectHeightPosition = [[dict objectForKey:kESPostItemRatioHeight]floatValue];
        
        if (touchImgViewX > itemXPosition && touchImgViewX < itemXPosition+itemRectWidthPosition && touchImgViewY > itemYPosition && touchImgViewY < itemYPosition+itemRectHeightPosition) {
        
            itemView = [arrItemView objectAtIndex:i];
            
            NSString *saleType = [dict_2 objectForKey:kESPostItemSaleType];
            itemView.tagLabel.text = [dict_2 objectForKey:kESPostItemHashTags];
            NSString *strItemPrice = [dict_2 objectForKey:kESPostItemPrice];
            if ([strItemPrice isEqualToString:@""] || strItemPrice == nil) {
                itemView.costtagLabel.text = @"";
            }else{
                if ([saleType isEqualToString:@"For Hire"]) {
                    itemView.costtagLabel.text = [NSString stringWithFormat:@"HIRE $%@",[dict_2 objectForKey:kESPostItemPrice]];
                }else if ([saleType isEqualToString:@"For Sale"]){
                    itemView.costtagLabel.text = [NSString stringWithFormat:@"BUY $%@",[dict_2 objectForKey:kESPostItemPrice]];
                }
                
            }
            
            
            PFFile *filteredImageFile = [dict_2 objectForKey:kESPostItemFilteredImage];
           
            [filteredImageFile getDataInBackgroundWithBlock:^(NSData * _Nullable data, NSError * _Nullable error){
                UIImage *image = [UIImage imageWithData:data];
                self.imageView.image = image;
            }];
            
            //========= item object owned count ============//
            dictOwnInfo        = [shared.arrOwnLabel objectAtIndex:i];
            // updated by //.//
            NSString *num = [dictOwnInfo objectForKey:kESOwnedCount];
            NSString *ownCount = [NSString stringWithFormat:@"%@ People own this",num];
...
- (void)logFrameSize{
//    NSLog(@"tempWidth --- %f", tempWidth);
//    NSLog(@"tempHeight --- %f", tempHeight);
    NSLog(@"image Width --- %f", _imageInfo.image.size.width);
    NSLog(@"image Height --- %f", _imageInfo.image.size.height);
    NSLog(@"imageview width --- %f", self.imageView.frame.size.width);
    NSLog(@"imageview height --- %f", self.imageView.frame.size.height);
